...
public class TestForCorrected {

    @Test
    public void Q1default(){
        CorrectedRunningAverage avg = new CorrectedRunningAverage();
    ...
    public void Q1explicit(double avgr,int numb) throws Exception {
        CorrectedRunningAverage avg = new CorrectedRunningAverage(avgr,numb);
        double result = avg.getCurrentAverage();
    ...
    public void Q2add(double first,double second,double third) throws Exception {
        CorrectedRunningAverage avg = new CorrectedRunningAverage(3, 5);
        ...
        assertAll(
                () -> assertTrue(result== actual && !finalExceptionThrown, "a)Adds and removes the given population to and from the CorrectedRunningAverage, respectively."),
                () -> assertEquals(copy, list, "b)Must not change the contents of the given List."),
                () -> assertTrue(finalNewavg == actual && finalNewavg == result, "c)Must return the new average."),
    ...
    public void Q3(int i) {
        ...
        boolean exceptionThrown = false;
        try {
            CorrectedRunningAverage avg = new CorrectedRunningAverage(5, i);
    ...
    public void Q4() throws Exception {
        CorrectedRunningAverage avg = new CorrectedRunningAverage(5, 1);
        boolean exceptionThrown = false;
    ...
    public void Q5(double avg1,int size1,double avg2,int size2) throws Exception {
        CorrectedRunningAverage temp1 = new CorrectedRunningAverage(avg1, size1);
        CorrectedRunningAverage temp2 = new CorrectedRunningAverage(avg2, size2);
        CorrectedRunningAverage result= new CorrectedRunningAverage();
        boolean exceptionThrown = false;
        try {
            result = CorrectedRunningAverage.combine(temp1, temp2);
        }
        catch (Exception e){
            exceptionThrown = true;
        }
        double actualavg = Math.round(1000.0*(avg1*size1 + avg2*size2)/(size1+size2))/1000.0;
        int actualsize = size1 + size2;
        CorrectedRunningAverage finalResult = result;
        boolean finalExceptionThrown = exceptionThrown;
        assertAll(
                () -> assertTrue(  finalResult instanceof CorrectedRunningAverage,"a)Must create a new CorrectedRunningAverage object"),
    ...
    @ParameterizedTest()
    @ValueSource(doubles = {4.9124712931248123,3.213124213213,6.214213214123,1.21243214123,3.14159})
    public void bonus(double i) throws Exception {
        CorrectedRunningAverage temp = new CorrectedRunningAverage(i,10);
        i = Math.round(i*1000.0)/1000.0;
        assertTrue(temp.getCurrentAverage()==i);
